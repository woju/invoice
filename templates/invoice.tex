%\definepapersize[A4e][width=230mm,height=317mm]
\enableregime[utf]
...
\setupparagraphs[customer][1][width=70mm]
...
\setupTABLE[c][2][width=60mm,align={nothyphenated,flushleft,lohi}]    % nazwa
\setupTABLE[c][3][width=15mm,align={nothyphenated,middle,lohi}]       % ilość + jm
\setupTABLE[c][4][width=25mm,align={nothyphenated,flushright,lohi}]   % netto
\setupTABLE[c][5][width=25mm,align={nothyphenated,flushright,lohi}]   % vat, wartość
\setupTABLE[c][6][width=25mm,align={nothyphenated,flushright,lohi}]   % brutto
...
\bTR[align={middle,lohi}]
    ...
    \bTD[nr=2]  {% trans %}amount{% endtrans %}\eTD
\eTR
\bTR[align={middle,lohi}]
...
\bTR
    ...
    \bTD {{ '%.2f'|format(line.price) }}\eTD
    \bTD {{ '%.2f'|format(line.vat) }}\eTD
    \bTD[nr=2] {{ line.brutto|format_currency(invoice.currency_code)|escapetex }}\eTD
\eTR
\bTR
    \bTD {{ '%.1f'|format(line.amount) }}\eTD
    \bTD {{ '%.2f'|format(line.netto) }} \eTD
    \bTD {{ '%.2f'|format(line.tax) }}\eTD
\eTR
{% endfor %}

\bTR[height=10mm]
    \bTD[nc=2,frame=off]\eTD
    \bTD[align={flushright,lohi}] {% trans %}total{% endtrans %}\eTD
    \bTD {{ '%.2f'|format(invoice.netto) }}\eTD
    \bTD {{ '%.2f'|format(invoice.tax) }}\eTD
    \bTD {{ invoice.brutto|format_currency(invoice.currency_code)|escapetex }}\eTD
\eTR

{% if invoice.currency.code != 'PLN' and invoice.tax > 0.0 %}
\bTR[height=10mm]
    \bTD[frame=off,nc=3] VAT po kursie śr. NBP z~dnia~{{ invoice.currency_rate_date.strftime('%d.%m.%Y') }} \eTD
    \bTD[align={flushright,lohi}] {{ invoice.currency_rate }}
        $\frac{\mathrm{ {{ invoice.currency_code }} }}{\mathrm{PLN}}$\eTD
    \bTD {{ invoice.tax_pln|format_currency('PLN')|replace(' ', '')|escapetex }}\eTD
    \bTD[frame=off] \eTD
\eTR
{% endif %}
...
\bTR
    ...
    \bTD[style={\tfa\bf}] {{ invoice.brutto|format_currency(invoice.currency.code)|escapetex }}\eTD
\eTR
\bTR
...
\bTR
    ...
    \bTD
        mBank S.A. FORMERLY BRE BANK S.A. (RETAIL BANKING) LODZ\crlf
...
\setupTABLE[c][1][align=flushleft,width=40mm]
